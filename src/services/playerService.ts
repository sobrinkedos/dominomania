    ...
    async getPlayerStats(playerId: string): Promise<PlayerStats> {
        try {
            ...
            const { data: wins, error: winsError } = await supabase
                .from(TableNames.GAME_PLAYERS)
                .select(`
                    id,
                    team,
                    ${TableNames.GAMES}!inner (id, team1_score, team2_score, status)
                `)
                .eq('player_id', playerId)
                .eq(`${TableNames.GAMES}.status`, 'finished')
                .or(`and(team.eq.1,${TableNames.GAMES}.team1_score.gte.6),and(team.eq.2,${TableNames.GAMES}.team2_score.gte.6)`);
            ...
            const { data: buchudas, error: buchudasError } = await supabase
                ...
                .select(`
                    id,
                    team,
                    ${TableNames.GAMES}!inner (id, is_buchuda, team1_score, team2_score)
                `)
                .eq('player_id', playerId)
                .eq(`${TableNames.GAMES}.is_buchuda`, true)
                .or(`and(team.eq.1,${TableNames.GAMES}.team1_score.gte.6),and(team.eq.2,${TableNames.GAMES}.team2_score.gte.6)`);
    ...
    async list(fetchStats = false) {
        try {
            ...
            const { data: myPlayers, error: myPlayersError } = await supabase
                .from(TableNames.PLAYERS)
                .select(`*, ${TableNames.USER_PLAYER_RELATIONS}(user_id, is_primary_user)`)
            ...
            const { data: communityPlayers, error: communityPlayersError } = await supabase
                ...
                .select(`
                    *,
                    ${TableNames.USER_PLAYER_RELATIONS}(user_id, is_primary_user),
                    ${TableNames.COMMUNITY_MEMBERS}!inner (
                        community_id,
                        ${TableNames.COMMUNITIES}!inner (
                            id,
                            ${TableNames.COMMUNITY_ORGANIZERS}!inner (
                                user_id
                            )
                        )
                    )
                `)
                .eq(`${TableNames.COMMUNITY_MEMBERS}.${TableNames.COMMUNITIES}.${TableNames.COMMUNITY_ORGANIZERS}.user_id`, userData.user.id)
