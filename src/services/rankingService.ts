...
export const rankingService = {
    async getTopPlayers(communityId?: string): Promise<PlayerRanking[]> {
        ...
        if (!communityMembers || communityMembers.length === 0) {
            console.log('RankingService: Nenhum usuário encontrado nas comunidades');
            return [];
        }

        // Extrair IDs únicos dos usuários
        const userIds = [...new Set(communityMembers
            .filter(member => member.users)
            .map(member => member.users.id))];

        // Buscar perfis dos usuários
        console.log('UserIds para busca:', userIds);
        
        // Como não temos estatísticas de jogos ainda, vamos retornar uma lista vazia
        console.log('RankingService: Sem estatísticas de jogos disponíveis');
        return [];
    ...
    async getTopPairs(communityId?: string): Promise<PairRanking[]> {
        ...
        // Como não temos estatísticas de duplas ainda, vamos retornar uma lista vazia
        console.log('RankingService: Sem estatísticas de duplas disponíveis');
        return [];
